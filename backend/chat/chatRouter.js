...
router.patch('/addMember/:id', async (req, res) => {
    ...
    try {
        ...
        if (!chat.groupalChat && chat.members.length > 0 && req.body.length > 1) return res.status(405).send({ error: 'Not allowed in one to one conversations' });

        if (!chat.groupalChat && chat.members.length === 0 && req.body.length > 2) return res.status(405).send({ error: 'Not allowed in one to one conversations' });
...
router.patch('/deleteMember/:id', async (req, res) => {
    ...
    try {
        ...
        if (!chat.groupalChat) return res.status(405).send({ error: 'Not allowed in one to one conversations' });
