...
router.patch('/addFriend', async (req, res, next) => {
    try {
        ...
        if (userA.friendsList.includes(userB.username)) return res.status(400).send({ error: 'Already in contact list' });
...
async function findUser(user) {
    const found = await UserModel.findOne({ $or: [{ email: user[0].email }, { username: user[0].username }] });
